...
import app.desktop.DesktopApp.Companion.showQuickstepWarningIfNecessary
import app.desktop.compat.DesktopQuickstepCompat
import app.desktop.data.AppDatabase
import app.desktop.data.wallpaper.service.WallpaperService
import app.desktop.factory.DesktopWidgetHolder
import app.desktop.gestures.GestureController
import app.desktop.gestures.VerticalSwipeTouchController
import app.desktop.gestures.config.GestureHandlerConfig
import app.desktop.nexuslauncher.OverlayCallbackImpl
...
import app.desktop.root.RootHelperManager
import app.desktop.root.RootNotAvailableException
import app.desktop.theme.ThemeProvider
import app.desktop.ui.popup.LauncherOptionsPopup
import app.desktop.ui.popup.DesktopShortcut
import app.desktop.util.getThemedIconPacksInstalled
import app.desktop.util.unsafeLazy
import app.desktop.views.DesktopFloatingSurfaceView
...
class DesktopLauncher : QuickstepLauncher() {
    ...
    private val statusBarClockListener = object : StateManager.StateListener<LauncherState> {
        override fun onStateTransitionStart(toState: LauncherState) {
            when (toState) {
                ...
                is AllAppsState,
                -> {
                    DesktopApp.instance.restoreClockInStatusBar()
                }
                else -> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        layoutInflater.factory2 = DesktopLayoutFactory(this)

        if (prefs.isFirstLoad.get()){
            prefs.isFirstLoad.set(false)
        }else{
            setSplashSta(prefs.isShowSplashSta.get())
        }
        ...
        if (prefs.autoLaunchRoot.get()) {
            lifecycleScope.launch {
                try {
                    RootHelperManager.INSTANCE.get(this@DesktopLauncher)
    ...
    override fun getSupportedShortcuts(): Stream<SystemShortcut.Factory<*>> = Stream.concat(
        ...
        Stream.concat(
            Stream.of(DesktopShortcut.UNINSTALL, DesktopShortcut.CUSTOMIZE),
            if (DesktopApp.isRecentsEnabled) Stream.of(DesktopShortcut.PAUSE_APPS) else Stream.empty(),
        ),
    )
    ...
    override fun handleGestureContract(intent: Intent?) {
        if (!DesktopApp.isRecentsEnabled) {
            ...
            if (gnc != null) {
                ...
                DesktopFloatingSurfaceView.show(this, gnc)
            }
        }
    }
    ...
    override fun createAppWidgetHolder(): LauncherWidgetHolder {
        val factory = LauncherWidgetHolder.HolderFactory.newFactory(this) as DesktopWidgetHolder.DesktopHolderFactory
    ...
    override fun makeDefaultActivityOptions(splashScreenStyle: Int): ActivityOptionsWrapper {
        ...
        val options = if (Utilities.ATLEAST_Q) {
            DesktopQuickstepCompat.activityOptionsCompat.makeCustomAnimation(
                this,
                0,
                0,
    ...
    private fun restartIfPending() {
        when {
            sRestartFlags and FLAG_RESTART != 0 -> desktopApp.restart(false)
    ...
    companion object {
        ...
        val instance get() = LauncherAppState.getInstanceNoCreate()?.launcher as? DesktopLauncher
    }
}

val Context.launcher: DesktopLauncher
    get() = BaseActivity.fromContext(this)

val Context.launcherNullable: DesktopLauncher? get() = try {
    launcher
