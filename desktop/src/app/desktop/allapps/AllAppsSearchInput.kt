...
import app.desktop.preferences2.subscribeBlocking
import app.desktop.qsb.AssistantIconView
import app.desktop.qsb.LawnQsbLayout.Companion.getLensIntent
import app.desktop.qsb.LawnQsbLayout.Companion.getSearchProvider
import app.desktop.qsb.ThemingMethod
import app.desktop.qsb.providers.Google
import app.desktop.qsb.providers.GoogleGo
import app.desktop.qsb.providers.PixelSearch
import app.desktop.qsb.setThemedIconResource
import app.desktop.search.DesktopRecentSuggestionProvider
import app.desktop.search.algorithms.DesktopSearchAlgorithm
import app.desktop.theme.drawable.DrawableTokens
import app.desktop.util.viewAttachedScope
...
class AllAppsSearchInput(context: Context, attrs: AttributeSet?) :
    ...
    private lateinit var apps: DesktopAlphabeticalAppsList<*>
    ...
    private val suggestionsRecent = SearchRecentSuggestions(launcher, DesktopRecentSuggestionProvider.AUTHORITY, DesktopRecentSuggestionProvider.MODE)
    ...
    override fun onFinishInflate() {
        ...
        input.onFocusChangeListener = OnFocusChangeListener { _, hasFocus ->
            ...
            if (hasFocus) {
                if (prefs2.searchAlgorithm.firstBlocking() != DesktopSearchAlgorithm.APP_SEARCH) {
        ...
        input.addTextChangedListener(
            ...
            afterTextChanged = {
                updateHint()
                if (input.text.toString() == "/desktopdebug") {
    ...
    override fun initializeSearch(appsView: ActivityAllAppsContainerView<*>) {
        apps = appsView.searchResultList as DesktopAlphabeticalAppsList<*>
        this.appsView = appsView
        searchBarController.initialize(
            DesktopSearchAlgorithm.create(context),
            input,
            launcher,
            this,
        )
