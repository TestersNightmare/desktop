...
import app.desktop.util.RecentHelper
import app.desktop.util.TaskUtilLockState
...
class TaskOverlayFactoryImpl(@Suppress("UNUSED_PARAMETER") context: Context) : TaskOverlayFactory() {
    ...
    class TaskOverlay(
    ...
    ) : TaskOverlayFactory.TaskOverlay<DesktopOverviewActionsView>(taskThumbnailView) {
        ...
        private inner class OverlayUICallbacksImpl(
            ...
            task: Task?,
        ) : TaskOverlayFactory.TaskOverlay<DesktopOverviewActionsView>.OverlayUICallbacksImpl(
            isAllowedByPolicy,
            task,
        ),
            OverlayUICallbacks {
