...
import app.desktop.preferences.PreferenceChangeListener
...
class DesktopASISearchAlgorithm(context: Context) :
    DesktopSearchAlgorithm(context),
...
//        searchSession!!.query(searchQuery, Executors.MAIN_EXECUTOR, null)
    ...
    private fun createSearchSession() {
        if (DesktopQuickstepCompat.ATLEAST_U) return
        Executors.UI_HELPER_EXECUTOR.execute {
            searchSession?.destroy()
