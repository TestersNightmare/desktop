...
import app.desktop.allapps.views.SearchItemBackground
import app.desktop.allapps.views.SearchResultView.Companion.EXTRA_QUICK_LAUNCH
import app.desktop.preferences2.PreferenceManager2
import app.desktop.search.DesktopSearchAdapterProvider
import app.desktop.search.adapter.SearchAdapterItem
import app.desktop.search.adapter.SearchTargetCompat
import app.desktop.search.adapter.SearchTargetCompat.Companion.RESULT_TYPE_APPLICATION
import app.desktop.search.adapter.SearchTargetCompat.Companion.RESULT_TYPE_SHORTCUT
...
sealed class DesktopSearchAlgorithm(
    ...
    protected fun transformSearchResults(results: List<SearchTargetCompat>): List<SearchAdapterItem> {
        val filtered = results
            ...
            .filter { DesktopSearchAdapterProvider.viewTypeMap[it.layoutType] != null }
    ...
    companion object {
        ...
        fun isASISearchEnabled(context: Context): Boolean {
            ...
            if (!DesktopApp.isRecentsEnabled) return false

            if (!ranCompatibilityCheck) {
                ranCompatibilityCheck = true
                DesktopASISearchAlgorithm.checkSearchCompatibility(context)
            }
            return true
        }

        fun create(context: Context): DesktopSearchAlgorithm {
            val prefs = PreferenceManager2.getInstance(context)
            val searchAlgorithm = prefs.searchAlgorithm.firstBlocking()

            return when {
                searchAlgorithm == ASI_SEARCH && isASISearchEnabled(context) -> DesktopASISearchAlgorithm(
                    context,
                )

                searchAlgorithm == LOCAL_SEARCH -> DesktopLocalSearchAlgorithm(context)
                else -> DesktopAppSearchAlgorithm(context)
            }
        }
    }
}
