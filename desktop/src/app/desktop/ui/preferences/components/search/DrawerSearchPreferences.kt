...
import app.desktop.ui.preferences.components.controls.MainSwitchPreference
...
import app.desktop.util.filesAndStorageGranted
...
fun DrawerSearchPreference(
    ...
    MainSwitchPreference(
        ...
        PreferenceGroup(heading = stringResource(id = R.string.show_search_result_types)) {
            ...
            if (searchAlgorithm != DesktopSearchAlgorithm.ASI_SEARCH) {
                val canDisable = searchAlgorithm != DesktopSearchAlgorithm.APP_SEARCH
            ...
            when (searchAlgorithm) {
                DesktopSearchAlgorithm.LOCAL_SEARCH -> {
                    LocalSearchSettings(
                        prefs = prefs,
                        prefs2 = prefs2,
                        context = context,
                ...
                DesktopSearchAlgorithm.ASI_SEARCH -> {
...
private fun SearchProvider(
    ...
    val searchAlgorithmEntries = remember {
        sequenceOf(
            ListPreferenceEntry(DesktopSearchAlgorithm.APP_SEARCH) { stringResource(R.string.search_algorithm_app_search) },
            ListPreferenceEntry(DesktopSearchAlgorithm.LOCAL_SEARCH) { stringResource(R.string.search_algorithm_global_search_on_device) },
            ListPreferenceEntry(DesktopSearchAlgorithm.ASI_SEARCH) { stringResource(R.string.search_algorithm_global_search_via_asi) },
        ).filter {
            when (it.value) {
                DesktopSearchAlgorithm.ASI_SEARCH -> DesktopSearchAlgorithm.isASISearchEnabled(
                    context,
                )
