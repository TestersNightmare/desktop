...
public class PreviewBackground extends DelegatedCellDrawing {
    ...
    private float getFolderBackgroundRadius() {
        Resources r = mContext.getResources();
        return r.getDimension(R.dimen.folder_background_radius);
    }
    ...
    public void setup(Context context, ActivityContext activity, View invalidateDelegate,
        ...
        mBgColor = ColorUtils.setAlphaComponent(mBgColor, DesktopUtilsKt.getFolderPreviewAlpha(context));
        ta.recycle();
    ...
    public void drawBackground(Canvas canvas) {
        ...
        // previewSize == 单个图标宽度
        float baseR = previewSize / 2f;
        // mScale 为动画缩放因子，0.9f 将背景宽度设为图标的 90%
        float r     = baseR * mScale * 0.9f;
        // 圆心仍以图标中心对齐
        float cx    = getOffsetX() + baseR;
        float cy    = getOffsetY() + baseR;

        RectF rect = new RectF(cx - r, cy - r, cx + r, cy + r);
        float radius = getFolderBackgroundRadius();
