        ...
        if (info != null) {
            ...
            try {
                ...
                if (
                    Utilities.ATLEAST_U && (
                            c.restoreFlag != 0 ||
                                    (isArchived && !targetPkg.isNullOrEmpty())
                            )
                ) {
                    tempPackageKey.update(targetPkg, c.user)
                    ...
                    if (si == null) {
                        info.runtimeStatusFlags =
                            info.runtimeStatusFlags and ItemInfoWithIcon.FLAG_INSTALL_SESSION_ACTIVE.inv()
                    } else if (activityInfo == null || isArchived) {
                        // 归档应用：带上进度（或设备重启后仍有挂起安装会话）
