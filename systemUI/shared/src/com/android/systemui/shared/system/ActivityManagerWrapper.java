...
import app.desktop.compatlib.RecentsAnimationRunnerCompat;
import app.desktop.compatlib.eleven.ActivityManagerCompatVR;
...
public class ActivityManagerWrapper {
    ...
    public List<ActivityManager.RecentTaskInfo> getRecentTasks(int numTasks, int userId) {
        return DesktopQuickstepCompat.getActivityManagerCompat().getRecentTasks(numTasks, userId);
    }
    ...
    public boolean startRecentsActivity(
        ...
        try {
            ...
            if (animationHandler != null) {
                runner = new RecentsAnimationRunnerCompat() {
                    ...
                    public void onAnimationCanceled(Object taskSnapshot) {
                        if (DesktopQuickstepCompat.ATLEAST_S) {
                            animationHandler.onAnimationCanceled(
                                    ThumbnailData.wrap(new int[]{0}, new TaskSnapshot[]{(TaskSnapshot) taskSnapshot}));
                        } else if (DesktopQuickstepCompat.ATLEAST_R) {
                            ActivityManagerCompatVR compat = (ActivityManagerCompatVR) DesktopQuickstepCompat.getActivityManagerCompat();
            ...
            DesktopQuickstepCompat.getActivityManagerCompat().startRecentsActivity(intent, eventTime, runner);
            return true;
        } catch (Exception e) {
            return false;
        }
